...
trait SecureSocial extends Controller {
   ...
   * @return
   */
  def SecuredAction[A](ajaxCall: Boolean, authorize: Option[Authorization], p: BodyParser[A])(f: SecuredRequest[A] => Result) = Action(p) { implicit request =>
    ...
    result.getOrElse {
      defaultResult(ajaxCall)
    }
  }

  private def defaultResult[A](ajaxCall: Boolean)(implicit request: Request[A]) = {
  ...
  def AsyncSecuredAction[A](ajaxCall: Boolean = false, authorize: Option[Authorization] = None)(f: SecuredRequest[AnyContent] => Future[SimpleResult]): Action[AnyContent] = Action.async { implicit request: Request[AnyContent] =>
    ...
    val maybeResult = for {
      ...
      if (authorize.isEmpty || authorize.get.isAuthorized(user)) {
        ...
        if (ajaxCall) {
          Future(ajaxCallNotAuthorized(request))
        } else {
          Future(Redirect(RoutesHelper.notAuthorized.absoluteURL(IdentityProvider.sslEnabled)))
        }
      }
    }

    maybeResult.getOrElse {
      Future(defaultResult(ajaxCall))
    }
  }
   ...
   * @return
   */
  def SecuredAction(ajaxCall: Boolean, authorize: Authorization)(f: SecuredRequest[AnyContent] => Result): Action[AnyContent] =
   ...
   * @return
   */
  def SecuredAction(authorize: Authorization)(f: SecuredRequest[AnyContent] => Result): Action[AnyContent] =
   ...
   * @return
   */
  def SecuredAction(ajaxCall: Boolean)(f: SecuredRequest[AnyContent] => Result): Action[AnyContent] =
